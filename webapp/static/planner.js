...
function showHub() {
  // TODO: Re-enable PIN screen when security is added back
...
function enterSection(id) {
  ...
  // Ensure sidebar and main are visible
...
function goToFront() {
  // Hide hub or section pages, return to front page
  const hub = document.getElementById('hub-page');
  hub.classList.add('hidden');
  setTimeout(() => hub.style.display = 'none', 700);
  const fp = document.getElementById('front-page');
  fp.style.display = 'flex';
  requestAnimationFrame(() => requestAnimationFrame(() => fp.classList.remove('hidden')));
}
...
function goToDashboard() {
  // Hide the main content area (sidebar + sections)
...
const NAV_ITEMS = ['overview','income','spending','budget','fixed','goals','investments','travel','calendar'];
...
// ── AI FINANCE INSIGHTS (preserved from repo) ──
